...
-- AND OPERATOR
select * from movies where imdb_rating <= 9 and	imdb_rating >= 6 ;

-- BETWEEN OPERTOR
select * from movies;
select * from movies where imdb_rating between 1 and 7;
 
-- OR OPERATOR
 select * from movies where release_year = 2022 or release_year = 2019 or release_year = 2018;
 
-- IN OPERATOR(also can be used with text query)
select * from movies where release_year in (2022,2019,2018);
select * from movies where studio in ("Marvel Studios","Hombale Films"); 

-- NULL and NOT NULL for Numeric Query.
